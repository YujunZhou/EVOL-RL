...
class SemanticTTRLRewardManager:
                            ...
                            if response_length > 0:
                                ...
                                if isinstance(old_log_probs, torch.Tensor) and old_log_probs.numel() > 0:
                                    ...
                                    if response_log_probs.numel() > 0:
                                        ...
                                        if first_success and self.debug_mode:
    ...
    def compute_post_ttrl_metrics(self, data: DataProto):
        ...
        assert len(data) % self.n_samples_per_prompt == 0, \
            f"Data length ({len(data)}) must be divisible by n_samples_per_prompt ({self.n_samples_per_prompt})"
        ...
        for prompt_i in range(prompt_num):
            group_vote_rewards = []  # Vote rewards
            group_pred_outputs = []  # Predicted outputs
            group_labels = []        # Ground truth
            group_extra_info = []    # Extra information
            task = None

            # Collect all samples for current prompt
            for i in range(self.n_samples_per_prompt):
                data_item = data[prompt_i * self.n_samples_per_prompt + i]
                
                # Parse prompt and response tokens
                prompt_idx = data_item.batch["prompts"]
                prompt_length = prompt_idx.shape[-1]
                valid_prompt_length = data_item.batch["attention_mask"][:prompt_length].sum()
                valid_prompt_idx = prompt_idx[-valid_prompt_length:]
                ...
                vote_reward = data_item.batch["acc"]  # Previously calculated accuracy
            ...
            # Calculate post-training metrics for current prompt group
    ...
    def _compute_ttrl_reward(self, data: DataProto):
        ...
        assert len(data) % self.n_votes_per_prompt == 0, \
            f"Data length ({len(data)}) must be divisible by n_votes_per_prompt ({self.n_votes_per_prompt})"
        
        prompt_num = len(data) // self.n_votes_per_prompt
        print(f"  Processing {prompt_num} prompts, each with {self.n_votes_per_prompt} candidate answers")
        ...
        for prompt_i in range(prompt_num):
            ...
            if self.use_semantic_novelty:
                ...
                final_rewards = self._compute_novelty_rewards(group_pred_outputs, base_rewards)
            else:
